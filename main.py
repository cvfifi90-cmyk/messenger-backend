...
def login(req: LoginRequest):
    ...
    if row[5] == 1: raise HTTPException(status_code=403, detail="Ваш аккаунт заблокирован навсегда.")
    ...
    try:
        while True:
            ...
            if is_user_banned(user_id):
                ...
                await websocket.close(); manager.disconnect(user_id); break
...
async def admin_ban_user(user_id: str, key: str):
    if key != ADMIN_SECRET_KEY: raise HTTPException(status_code=403)
    ...
    if user_id in manager.active_sockets:
        ...
        try:
            await ws.send_text(json.dumps({"type": "user_banned", "message": "🚨 Ваш аккаунт заблокирован навсегда."}))
            await asyncio.sleep(0.1)
            await ws.close()
        except: pass
